...
void sim_reg_stats(struct stat_sdb_t *sdb)
{
	stat_reg_counter(sdb, "sim_num_insn",
			"total number of instructions executed", &sim_num_insn,
			sim_num_insn, NULL);
	stat_reg_counter(sdb, "sim_num_refs",
			"total number of loads and stores executed", &sim_num_refs, 0,
			NULL);
	...
	stat_reg_counter(sdb, "sim_num_icache_miss",
			"total number of instruction cache misses", &g_icache_miss, 0,
			NULL);
...
void cache_access(struct cache *c, unsigned addr, counter_t *miss_counter)
{
	unsigned index, tag;
	int i, k;
	unsigned int min = max_insts;
	index = (addr >> c->m_set_shift) & c->m_set_mask;
	tag = (addr >> c->m_tag_shift);
	assert(index < c->m_total_blocks);
	for (i = 0; i < c->m_nways; i++)
	{
		if ((c->m_tag_array[index + i].m_valid
				&& (c->m_tag_array[index + i].m_tag == tag)))
		{
			(&(c->m_tag_array[index + i]))->m_timestamp = sim_num_insn;
			break;
		}
		...
		else
		{
			...
			if (i == (c->m_nways - 1))
			{
				*miss_counter = *miss_counter + 1;
				c->m_tag_array[index + k].m_valid = 1;
				c->m_tag_array[index + k].m_tag = tag;
			}
		}
	}
}
...
#include "machine.def"
		...
		if (verbose)
		{
			myfprintf(stderr, "%10n [xor: 0x%08x] @ 0x%08p: ", sim_num_insn,
					md_xor_regs(&regs), regs.regs_PC);
