...
SRCS =	main.c sim-scalar-eece476.c  \
	memory.c regs.c  \
...
sim-scalar-eece476$(EEXT):	sysprobe$(EEXT) sim-scalar-eece476.$(OEXT) $(OBJS) libexo/libexo.$(LEXT)
	$(CC) -o sim-scalar-eece476$(EEXT) $(CFLAGS) sim-scalar-eece476.$(OEXT) $(OBJS) libexo/libexo.$(LEXT) $(MLIBS)
...
sim-tests sim-tests-nt: sysprobe$(EEXT) $(PROGS)
	...
	$(MAKE) "MAKE=$(MAKE)" "RM=$(RM)" "ENDIAN=$(ENDIAN)" tests \
		"DIFF=$(DIFF)" "SIM_DIR=.." "SIM_BIN=sim-scalar-eece476$(EEXT)" \
		"X=$(X)" "CS=$(CS)" $(CS) \
	cd ..
...
sim-scalar-eece476.$(OEXT): host.h misc.h machine.h machine.def regs.h memory.h
sim-scalar-eece476.$(OEXT): options.h stats.h eval.h loader.h syscall.h sim.h
